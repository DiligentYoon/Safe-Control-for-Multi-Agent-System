...
def draw_frame(ax_gt, ax_belief, env, viz_data: dict):
    ...
    for i in range(env.num_agents):
        ...
        radius_min_px = env.cfg.d_safe * 0.5 / spec.res_m
        radius_max_px = env.cfg.d_max * 0.5 / spec.res_m
        ...
        # Get sensor properties from the environment config
        half = math.radians(env.cfg.sensor_fov_deg / 2.0)
        angles = np.linspace(-half, half, 20)
        poly_world = [(robot.x, robot.y)] + [
            (robot.x + env.cfg.sensor_max_range * math.cos(robot.yaw + a),
             robot.y + env.cfg.sensor_max_range * math.sin(robot.yaw + a)) for a in angles
        ]
        ...
        # --- Target point (if available in viz_data) ---
        target = viz_data.get("targets_world", [None]*env.num_agents)[i]
...
def plot_control_inputs(nominal_history, safe_history, dt, num_agents, save_path: str = None):
    ...
    for i in range(num_agents):
        ...
        ax_v = axes[i, 0]
        ax_v.plot(timesteps, nom_v, 'b--', label='v_nominal'); ax_v.plot(timesteps, safe_v, 'b-', label='v_safe')
        ax_v.set_title(f'Agent {i} - Linear Velocity'); ax_v.set_ylabel('v (m/s)'); ax_v.legend(); ax_v.grid(True, which='both', linestyle='--', linewidth=0.5)
        ax_w = axes[i, 1]
        ax_w.plot(timesteps, nom_w, 'r--', label='w_nominal'); ax_w.plot(timesteps, safe_w, 'r-', label='w_safe')
